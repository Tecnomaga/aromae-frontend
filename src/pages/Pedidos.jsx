...
export default function Pedidos() {
  ...
  const pedidosFiltrados = pedidos.filter((p) => filtroStatus === 'todos' || p.status === filtroStatus)
    .filter((p) => !filtroData || new Date(p.criadoEm).toISOString().slice(0, 10) === filtroData)
    .filter((p) => !busca || p.cliente?.nome?.toLowerCase().includes(busca.toLowerCase()) || p._id.includes(busca));
  ...
  if (loading) return (
    <div className="p-8 text-center">
      ...
      <p className="text-texto/50 text-sm">Carregando pedidos...</p>
    </div>
  );
    ...
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        ...
        <div className="flex gap-3 flex-wrap">
          <button onClick={() => window.open('/api/relatorios/pedidos', '_blank')} className="flex items-center gap-2 bg-gray-100 text-texto px-4 py-2 rounded-lg font-semibold hover:bg-gray-200 transition"><FileCsv size={20} /> Exportar CSV</button>
          <Link to="/pedidos/novo" className="flex items-center gap-2 bg-primaria text-white px-4 py-2 rounded-lg font-semibold hover:bg-primaria/90 transition"><Plus size={20} /> Novo Pedido</Link>
        </div>
      </div>
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="relative flex-1"><MagnifyingGlass size={20} className="absolute left-3 top-1/2 -translate-y-1/2 text-texto/40" /><input type="text" placeholder="Buscar por cliente ou ID..." value={busca} onChange={(e) => setBusca(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:border-primaria" /></div>
        <select value={filtroStatus} onChange={(e) => setFiltroStatus(e.target.value)} className="px-4 py-2 border border-gray-200 rounded-lg bg-white"><option value="todos">Todos os status</option>{Object.entries(statusMap).map(([v, { label }]) => <option key={v} value={v}>{label}</option>)}</select>
        <input type="date" value={filtroData} onChange={(e) => setFiltroData(e.target.value)} max={new Date().toISOString().slice(0, 10)} className="px-4 py-2 border border-gray-200 rounded-lg bg-white" />
      </div>
